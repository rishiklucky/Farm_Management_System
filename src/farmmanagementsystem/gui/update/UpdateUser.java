...
public class UpdateUser extends javax.swing.JDialog {
    ...
    private void jButtonSAVEActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonSAVEActionPerformed
        ...
        User temp = getUser();
        if(user.equals(temp)){
            JOptionPane.showMessageDialog(null, "NO CHANGES", "INVALID UPDATE", JOptionPane.ERROR_MESSAGE);
            return;
        }

        if(DBQuery.isExistUserID(temp.getUser_id()) && !temp.getUser_id().equalsIgnoreCase(user.getUser_id())){
            JOptionPane.showMessageDialog(null, "User ID Already Exist", "INVALID User ID", JOptionPane.ERROR_MESSAGE);
            return;
        }
        if(!checkAdministrator(user, temp)){
            JOptionPane.showMessageDialog(null, "You are the only \n Active Administrator.", "INVALID Update", JOptionPane.ERROR_MESSAGE);
            return;
        }
        if(confirm("UPDATE USER?")){
            DBQuery.updateUser(user, temp);
            DBQuery.insertLog(User_ID, recordLog(user, temp));
            JOptionPane.showMessageDialog(null, "SAVED", "User Update", JOptionPane.INFORMATION_MESSAGE);
            dispose();
        }
    ...
    */
    ...
    private void setUser(){
        ...
        jTextFieldFname.setText(user.getFname());
        jTextFieldLname.setText(user.getLname());
        jTextFieldMname.setText(user.getMname());
        jTextFieldUserID.setText(user.getUser_id());
        jRadioButtonActive.setSelected(user.isActive());
        jRadioButtonIN_Active.setSelected(!user.isActive());
        jComboBoxUserType.setSelectedItem(user.getUserType());
    }

    private boolean checkAdministrator(User old, User NEW){
        if(old.getUserType().contains("Sec"))
            return true;
        ArrayList<User> l = DBQuery.getUsers();
        for(int i=0; i<l.size();i++){
            if(l.get(i).getUserType().contains("Sec")){
                l.remove(i);i--;
            }else if(l.get(i).getUserType().contains("Admin") && !l.get(i).isActive()){
                l.remove(i);i--;
            }
        }
        if(l.size()>1)
            return true;
        if(!l.get(0).getUser_id().contentEquals(old.getUser_id()))
            return true;

        if(old.getUserType().contains("Admin") && !NEW.getUserType().contains("Admin")){            
            return false;
        }
        if(old.getUserType().contains("Admin") && !NEW.isActive())
        return false;
        return true;
    }

    private String recordLog(User old,User NEW){
        String activity = "Update User("+old.getUser_id()+") ";
        activity += old.getChangesFrom(NEW);
        return activity;
    }
}
