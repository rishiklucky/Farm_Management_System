...
/**
 *
 * @author The Jamaicanz
 */
public class GenerateTablePrintable {    
    
     public static Printable getPrintable(JTable jtable,PrintMode printMode, String headerFormat, String footerFormat){

                     return new TablePrintable(jtable, printMode, headerFormat, footerFormat);                      
    }
       
    public static void setCustomPrintable(JTable table){
    ...
    public static class TablePrintable implements Printable {
...
public TablePrintable(JTable table, JTable.PrintMode printMode, String headerformat, String footerformat) {
        ...
        this._headerFormat = headerformat;
        this._footerFormat = footerformat;
    ...
    private void printText(Graphics2D g2d, String[] lines, Rectangle2D rect, Font font, int imgWidth) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (rect.getWidth() < imgWidth) {                
                ...
                }else if(lines[i].startsWith("(tab)")){
                    lines[i] = lines[i].substring(5);
                    tx +=100;
                }else if(lines[i].startsWith("(halftab)")){
...
//                tx = -(int) (Math.ceil(rect.getWidth()) - imgWidth);
           ...
           // System.out.println(lines[i]);
            ...
            if(lines[i].contains("(tab)") && OccurFirst(lines[i]) == tab){
                String temp = lines[i].substring(0, lines[i].indexOf("(tab)"));                
                g2d.drawString(temp, tx, ty);
                lines[i] = lines[i].replaceAll(temp, "");
                i--;
            }else if(lines[i].contains("(halftab)")){
                String temp = lines[i].substring(0, lines[i].indexOf("(halftab)"));
                g2d.drawString(temp, tx, ty);
                lines[i] = lines[i].replaceAll(temp, "");
                i--;
            }
    ...
    int tab = 1, halftab =2, noTab =3;
    
    private int OccurFirst(String str){
        if(str.indexOf("(tab)")>=0 && str.indexOf("(halftab)")>=0){
            if(str.indexOf("(tab)") < str.indexOf("(halftab)")){
                return tab;
            }else{
                return halftab;                        
            }
