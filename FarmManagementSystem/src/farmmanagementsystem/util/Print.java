...
//        Printable   p = table.getPrintable(JTable.PrintMode.FIT_WIDTH, fs, fd);
        ...
        PageFormat f = new PageFormat();         
            ...
            pa.setImageableArea(0.5 * 72, 0.5 * 72, 6.5 * 72, 12 * 72);
            f.setPaper(pa);            
...
//        System.out.println(f.getPaper().getWidth()+" "+f.getPaper().getHeight());
    ...
    private void generateFormPreview() {
...
//        System.out.println(f.getPaper().getWidth()+" "+f.getPaper().getHeight());
     ...
     */
    public void showPreview( JTable tab,Container parent) {
        if (tab == null) {
    ...
    public void showFormPreview(  JPanel pan,Container parent) {
...
//        Printable   p = table.getPrintable(JTable.PrintMode.FIT_WIDTH, fs, fd);
        ...
        PageFormat f = new PageFormat();         
            ...
            Paper pa = new Paper();
            pa.setSize((8.5 * 72), (13 * 72));
            pa.setImageableArea(0, 0,(8.5 * 72), (13 * 72));
            f.setPaper(pa);            
            f.setOrientation(PageFormat.PORTRAIT);
            Pformat = f;
...
//
     ...
     */
    public boolean printClicked = false;
    public void printComponent() {

        if (table == null) {printClicked = true;
            ...
            try {
                pr.print();
                JOptionPane.showMessageDialog(null, "Document Printed", "Printing", JOptionPane.INFORMATION_MESSAGE);
            } catch (PrinterException ex) {
                ex.printStackTrace();
            }
        } else {printClicked = true;
                    PrinterJob pr = PrinterJob.getPrinterJob();
                    pr.pageDialog(Pformat);
                    pr.setPrintable(pri);
                try {
